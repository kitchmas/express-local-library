...
exports.bookinstance_create_get = function(req, res, next) {
    ...
    Book.find({}, 'title')
        .exec(function(err, books) {
            if (err) { return next(err); }
            //Succesful, so render
            res.render('bookinstance_form', { title: 'Create BookInstance', statuses: ['Available', 'Maintenance', 'Loaned', 'Reserved'], book_list: books });
        });
};
    ...
    if (errors) {
        ...
        Book.find({}, 'title')
            .exec(function(err, books) {
                if (err) { return next(err); }
                //Successful, so render
                res.render('bookinstance_form', { title: 'Create BookInstance', statuses: ['Available', 'Maintenance', 'Loaned', 'Reserved'], book_list: books, selected_book: bookinstance.book._id, errors: errors, bookinstance: bookinstance });
            });
        return;
    } else {
...
exports.bookinstance_delete_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        if (err) { return next(err); }
        //Successful, so render
        res.render('book_instance_delete', { title: 'Delete Book Instance', book_instance: results.book_instance });
    });
